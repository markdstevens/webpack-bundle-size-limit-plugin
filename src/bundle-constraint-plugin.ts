...
class BundleConstraintPlugin {
  ...
  private filterAssetByFileExtension(file: string) {
  ...
  private getConfig(fileName: string, config: Config, compilation: Compilation, options: BundleConstraintPluginOptions) {
    ...
    if (match && match.length) {
      if (match.length === 1) {
        return config?.bundles.filter(bundle => new RegExp(bundle.name).toString() === match[0].regex.toString())[0]
      }
      compilation.errors.push(error(`File "${fileName}" matches multiple patterns: ${match.map(config => `"${config.fileName}"`).join(', ')}`));
      return null;
    }
  ...
  private fromByteToX(numBytes: number, unit: string) {
    return `${Math.round((numBytes / fileSizeDenominations[unit]) * 100) / 100}${unit}`;
  }

  apply(compiler: Compiler) {
    compiler.hooks.emit.tapAsync('BundleConstraintPlugin', (compilation: Compilation, callback: any) => {
          ...
          for (const asset in compilation.assets) {
            if (!this.filterAssetByFileExtension(asset)) {
              const fileWithConfig = {
                fileName: asset,
                size: parseFloat(execSync(`wc -c dist/${asset} | awk '{$1=$1};1' | cut -d$' ' -f1`).toString().trim()),
              ...
              if (fileWithConfig.config && fileWithConfig.size > fileWithConfig.config.maxSizeInBytes) {
                compilation.errors.push(error(`Bundle size exceeded.
